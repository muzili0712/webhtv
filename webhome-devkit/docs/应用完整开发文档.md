...
> 当前状态：`dev` 分支尚未实现端到端的观影记录删除同步。

现有 `/api/playback/progress/delete` 只用于外部程序删除某一台设备的本地历史。历史页单条删除或清空不会生成出站删除事件，远端拉取也只处理普通进度 upsert，因此服务端旧记录可能在后续同步中把本地已删除记录重新创建。

可靠的多端删除同步需要客户端和历史存储服务共同支持删除墓碑：客户端记录并上报 `deletedAt`，服务端持久化墓碑并在同步查询中返回，其他客户端按 upsert 的 `updatedAt` 与墓碑的 `deletedAt` 解决冲突。不能根据远端列表中缺少某条记录直接推断删除。

当前分支还支持全局历史和 TMDB 聚合，一个展示项可能对应多个 `cid/configKey` 下的底层记录。后续实现必须为全部实际删除记录生成墓碑；全局清空也必须按实际 `configKey` 分区处理。

完整现状、协议建议、前后端改造范围和验证清单见 [《观影记录删除同步设计与前后端边界》](../../docs/playback-history-delete-sync-design.md)。
